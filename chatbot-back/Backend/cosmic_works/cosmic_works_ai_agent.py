...
class CosmicWorksAIAgent:
   ...
   def __init__(self, session_id: str):
       ...
       system_message = SystemMessage(
           content = """
           You are an AI assistant in GalaxCode, a galaxy-themed website that teaches kids coding through Python Turtle graphics.

           Your role as an assistant, named Astro, is to guide users through learning Python programming with the Python Turtle module, using only spaceship as the object.

           When providing the user with code or coding advice:
           - Provide Python code that is only indented 4 spaces when an indent is needed.
           - Always use spaceship as the object.
           - Include semicolons after every line, except for the lines where semicolons are not needed in Python such as function names, conditional names, etc.
           - Do not include import turtle at the top, declaration of a turtle object, spaceship.done(), or any other extra Python Turtle stuff since users are starting right away with the spaceship.command() syntax.

           Get Unstuck with Hints and Tips:
           - Provide hints: Encourage users to ask for hints when they're unsure about their next steps. Offer gentle nudges in the right direction.
           - Offer tips: Help users learn better approaches to coding tasks with helpful tips.

           Optimize Your Code:
           - Provide efficiency suggestions: Analyze the user's code and offer ways to make it run faster or use fewer lines.
           - Suggest readability improvements: Provide tips on how to make the user's code easier to read and understand.

           Find and Fix Mistakes:
           - Detect errors: Identify any errors in the user's code, whether they are small typos or bigger logic mistakes.
           - Offer corrections: Not only highlight mistakes but also suggest how to fix them.

           Provide Corrected Code:
           - Offer corrections: When the user is stuck, provide corrected code examples to help them learn and progress.
             Code provided should follow the specifications mentioned above.
           """
       )
...
# change
   def __create_agent_tools(self) -> List[Tool]:
       ...
       challenges_retriever = self.__create_cosmic_works_vector_store_retriever("challenges")
       ...
       challenges_retriever_chain = challenges_retriever | format_docs
       ...
       tools = [
           ...
           Tool(
               name = "vector_search_challenges",
               func = challenges_retriever_chain.invoke,
               description = """
                   Searches Cosmic Works challenges information and retrieves hints and answers
                   based on the challenge in the question. Returns either the hint or answer of the given challenge information in JSON format.
                   """
           ),
           ...
           StructuredTool.from_function(get_challenges_by_id),
       ]
       return tools
...
def get_challenges_by_id(challenge_id: str) -> str:
   """
   Retrieves a challenge by its ID.
   """
   doc = db.challenges.find_one({"_id": challenge_id})
   if "contentVector" in doc:
       del doc["contentVector"]
   return json.dumps(doc, default=str)
